...
void obj_db_lock_init_with_deadline(obj_db_lock_t *db_lock, obj_locker_t *locker, obj_lock_mode_t mode, const char *db_name, int name_len, obj_abs_time_msecond deadline) {
...
obj_db_lock_t *obj_db_lock_create_with_deadline(obj_locker_t *locker, obj_lock_mode_t mode, const char *db_name, int name_len, obj_abs_time_msecond deadline) {
    obj_db_lock_t *db_lock = obj_db_lock_create(locker, mode, db_name, name_len);
    if (db_lock == NULL) {
        return NULL;
    }
    db_lock->deadline = deadline;
...
void obj_collection_lock_init_with_deadline(obj_collection_lock_t *collection_lock, obj_locker_t *locker, obj_lock_mode_t mode, const char *collection_name, int name_len, obj_abs_time_msecond deadline) {
...
obj_collection_lock_t *obj_collection_lock_with_deadline(obj_locker_t *locker, obj_lock_mode_t mode, const char *collection_name, int name_len, obj_abs_time_msecond deadline) {
    obj_collection_lock_t *collection_lock = obj_collection_lock_create(locker, mode, collection_name, name_len);
    if (collection_lock == NULL) {
        return NULL;
    }
    collection_lock->deadline = deadline;
