...
static obj_skiplist_node_t *obj_skiplist_node_create(obj_bson_t *key, obj_record_t *record);
static void obj_skiplist_node_destroy(obj_skiplist_node_t *node);
static int obj_skiplist_random_level();
static obj_bool_t obj_skiplist_locate(obj_skiplist_t *skiplist, obj_bson_t *key, obj_skiplist_node_t **out);
static obj_skiplist_node_t *obj_skiplist_advance_to(obj_skiplist_t *skiplist, obj_skiplist_node_t *current, obj_bson_t *key);
...
static obj_skiplist_t *obj_skiplist_create() {
    ...
    skiplist->level = 1;
    skiplist->length = 0;
    skiplist->head = obj_skiplist_node_create(NULL, NULL);
    int i;
...
/* seek end cursor */
inline void obj_index_iterator_seek_end_cursor(obj_index_iterator_t *iter) {
    if (!obj_index_iterator_has_end_state(iter)) {
        return;
    }
    obj_skiplist_locate(iter->skiplist, iter->end_state.key, &iter->end_state.node);
}
...
obj_index_key_entry_t obj_index_iterator_seek(obj_index_iterator_t *iter, obj_bson_t *key) {
    obj_bool_t can_use_advance = false;
    if (obj_index_iterator_is_eof(iter)) {
        return obj_index_iterator_curr(iter);
    }
    int cmp = obj_index_key_compare(iter->cur_node->key, key, iter->skiplist->order);
    can_use_advance = (cmp < 0);
    if (can_use_advance) {
        iter->cur_node = obj_skiplist_advance_to(iter->skiplist, iter->cur_node, key);
    } else {
        obj_skiplist_locate(iter->skiplist, key, &iter->cur_node);
    }
